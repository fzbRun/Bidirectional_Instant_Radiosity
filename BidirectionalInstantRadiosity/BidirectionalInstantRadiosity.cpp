...
myCamera camera(glm::vec3(0.0f, 1.0f, 4.0f));
float lastTime = 0.0f;
...
private:
	...
	VkPipelineLayout BIRPipelineLayout;
	VkPipeline BIRPipeline;
	std::vector<VPL> vpls;
	std::vector<SampleVertex> sampleVertices;
	std::vector<VPL> resampleVPLs;
	...
	void createBuffers() {
		...
		//BIR参数的存储buffer
		for (int i = 0; i < VPLNum * 2; i++) {
			vpls.push_back(VPL());
		}
		my_buffer->createStaticBuffer(my_device->physicalDevice, my_device->logicalDevice, my_device->graphicsQueue, sizeof(VPL) * VPLNum * 2, &vpls);

		for (int i = 0; i < VPLNum; i++) {
			sampleVertices.push_back(SampleVertex());
		}
		my_buffer->createStaticBuffer(my_device->physicalDevice, my_device->logicalDevice, my_device->graphicsQueue, sizeof(SampleVertex) * VPLNum, &sampleVertices);

		//BIR参数的存储buffer
		for (int i = 0; i < resampleVPLNum; i++) {
			resampleVPLs.push_back(VPL());
		}
		my_buffer->createStaticBuffer(my_device->physicalDevice, my_device->logicalDevice, my_device->graphicsQueue, sizeof(VPL) * resampleVPLNum, &resampleVPLs);
	...
	void createMyDescriptor() {
		...
		uint32_t storageBufferNumAllLayout = 7;
		...
		//VPL
		VkDescriptorBufferInfo VPLBufferInfo{};
		VPLBufferInfo.buffer = my_buffer->buffersStatic[4];
		VPLBufferInfo.offset = 0;
		VPLBufferInfo.range = sizeof(VPL) * VPLNum * 2;
		...
		descriptorWrites[6].pBufferInfo = &VPLBufferInfo;

		//sampleVertices
		VkDescriptorBufferInfo SampleVerticesBufferInfo{};
		SampleVerticesBufferInfo.buffer = my_buffer->buffersStatic[5];
		SampleVerticesBufferInfo.offset = 0;
		SampleVerticesBufferInfo.range = sizeof(SampleVertex) * VPLNum;
		...
		descriptorWrites[7].pBufferInfo = &SampleVerticesBufferInfo;

		//resampleVPL
		VkDescriptorBufferInfo resampleVPLBufferInfo{};
		resampleVPLBufferInfo.buffer = my_buffer->buffersStatic[6];
		resampleVPLBufferInfo.offset = 0;
		resampleVPLBufferInfo.range = sizeof(VPL) * resampleVPLNum;
		...
		descriptorWrites[8].pBufferInfo = &resampleVPLBufferInfo;
	...
	void createGraphicsPipeline() {
		...
		auto shadowVertShaderCode = readFile("C:/Users/fangzanbo/Desktop/渲染/rayTracing/BidirectionalInstantRadiosity/BidirectionalInstantRadiosity/shaders/shadowVert.spv");
		auto shadowFragShaderCode = readFile("C:/Users/fangzanbo/Desktop/渲染/rayTracing/BidirectionalInstantRadiosity/BidirectionalInstantRadiosity/shaders/shadowFrag.spv");
		...
		auto sceneVertShaderCode = readFile("C:/Users/fangzanbo/Desktop/渲染/rayTracing/BidirectionalInstantRadiosity/BidirectionalInstantRadiosity/shaders/sceneVert.spv");
		auto sceneFragShaderCode = readFile("C:/Users/fangzanbo/Desktop/渲染/rayTracing/BidirectionalInstantRadiosity/BidirectionalInstantRadiosity/shaders/sceneFrag.spv");
	...
	void createComputePipeline() {
		auto computeShaderCode = readFile("C:/Users/fangzanbo/Desktop/渲染/rayTracing/BidirectionalInstantRadiosity/BidirectionalInstantRadiosity/shaders/BidirectionalInstantRadiosity.spv");
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		for (uint32_t i = 0; i < randomXYNum; i++) {
			ubo.randomXY[i] = glm::vec4(Hammersley(i, randomXYNum), 0.0f, 0.0f);
		}
